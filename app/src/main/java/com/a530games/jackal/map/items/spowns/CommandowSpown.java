...
public class CommandowSpown extends MapCell implements EnemyDieEventHandler
{
    private final Texture texture;

    private Commandos commandos = null;

    public CommandowSpown(int col, int row)
    {
        super(col, row);

        this.texture = new SpownTexture(SpownTexture.SpownType.commandos);
    }
    ...
    public void update(float deltaTime, CellEventCallbackHandler callbackHandler)
    {
        if (this.commandos == null) {
            this.commandos = new Commandos(this.getCenter());
